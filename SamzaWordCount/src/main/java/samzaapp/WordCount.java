...
//  private static final String OUTPUT_STREAM_ID = "word-count-output";
  ...
  public void describe(StreamApplicationDescriptor streamApplicationDescriptor) {
    ...
    lines.filter(kv -> kv.value.contains("hi")).sendTo(counts);

//    OutputStream<String> myOutStream = new OutputStream<>();
//    lines
//            .map(kv -> kv.value)
//            .flatMap(s -> Arrays.asList(s.split("\\W+"))).sink(msg -> {System.out.println(msg);});

//    lines
//            .map(kv -> kv.value)
//            .flatMap(s -> Arrays.asList(s.split("\\W+"))).sink();

//    MessageStream<String> r = lines
//            .map(kv -> kv.value)
//            .flatMap(s -> Arrays.asList(s.split("\\W+")));
//    r.sink((msg, msgCollector, taskCoord) -> {
